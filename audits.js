...
// QA Auditors are identified by email (QA_AUDITOR_EMAILS env, comma/space separated). Any
// auditor not on this list is treated as a Team Lead. This lets every audit be tagged TL vs
// QA and the two scores kept separate without a per-person sheet change.
const QA_AUDITOR_EMAILS = String(process.env.QA_AUDITOR_EMAILS || process.env.VITE_QA_AUDITOR_EMAILS || '')
  .split(/[,\s]+/).map(s => s.trim().toLowerCase()).filter(Boolean);
// Fallback to the known QA Auditor when the env var is unset, so TL/QA tagging works out of
// the box. Set QA_AUDITOR_EMAILS in the environment to override.
if (!QA_AUDITOR_EMAILS.length) QA_AUDITOR_EMAILS.push('samapti.pal@solarsquare.in');
...
const AUDIT_HEADERS = [
  ...
  'Auditor Name', 'Auditor Email', 'Auditor Role', 'Audit Date', 'Overall %', 'Section Scores (JSON)', 'Answers (JSON)',
...
function rowToRecord(r) {
  ...
  return {
    ...
    auditorRole: String(r['Auditor Role'] || '').trim() || roleForEmail(r['Auditor Email']),
    callDate,
...
async function handleGet() {
  const rows = rowsToObjects(await readSheet('Audits!A:Z'));
...
async function handlePost(event, verifiedUser) {
  ...
  const auditorRole = (p.auditorRole === 'QA' || p.auditorRole === 'TL') ? p.auditorRole : roleForEmail(auditorEmail);
  ...
  const rowObj = {
    ...
    'Auditor Name': auditorName, 'Auditor Email': auditorEmail, 'Auditor Role': auditorRole,
  ...
  return json(200, { success: true, id, overall: sc.overall, auditorRole, email: emailNote });
}
